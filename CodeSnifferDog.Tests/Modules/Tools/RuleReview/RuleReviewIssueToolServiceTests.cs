...
[TestClass]
public sealed class RuleReviewIssueToolServiceTests
{
    ...
    public async Task CreateGetListUpdateDeleteIssueAsync_UsesScopedStoreAndMapsResults()
    {
        ...
        RuleReviewIssueToolService service = new(new InMemoryRuleReviewIssueStore(), ruleFlowKey);

        CreateRuleReviewIssueResult created = await service.CreateRuleReviewIssueAsync(
            CreateIssueArgs(" high ", " Program.cs "),
            TestContext.CancellationToken);
        StoredRuleReviewIssue fetched = await service.GetRuleReviewIssueAsync(
            new GetRuleReviewIssueArgs
            {
                RuleReviewIssueId = $" {created.RuleReviewIssueId} ",
            },
            TestContext.CancellationToken);
        StoredRuleReviewIssue updated = await service.UpdateRuleReviewIssueAsync(
            ...
            TestContext.CancellationToken);
        IReadOnlyList<StoredRuleReviewIssue> issues = await service.ListRuleReviewIssuesAsync(TestContext.CancellationToken);
    ...
    {
        RuleReviewIssueToolService service = new(
    ...
    public async Task ListRuleReviewIssuesAsync_IsolatedByRuleFlowKey()
    {
        ...
        RuleReviewIssueToolService first = new(store, CreateRuleFlowKey("task-1"));
        RuleReviewIssueToolService second = new(store, CreateRuleFlowKey("task-2"));
