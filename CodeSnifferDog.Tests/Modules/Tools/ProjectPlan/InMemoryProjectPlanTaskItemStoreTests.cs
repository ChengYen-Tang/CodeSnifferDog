...
[TestClass]
public sealed class InMemoryProjectPlanTaskItemStoreTests
{
    [TestMethod]
    ...
    public async Task BeginAttempt_Restore_BlocksLateWritesFromTimedOutAttempt()
    {
        ...
        lease.Restore();
        StoredProjectPlanTaskItem generatedTaskItem = await AgentRunAttemptContext.RunAsync(attemptId, async () =>
    ...
    {
        InMemoryProjectPlanTaskItemStore store = new();
        StoredProjectPlanTaskItem taskItem = await store.AddAsync(CreateTaskItem("Program.cs"), CancellationToken.None);
        Guid attemptId = Guid.NewGuid();
        IAgentAttemptLease lease = store.BeginAttempt(attemptId);
        lease.Restore();
    ...
    private static ProjectPlanTaskItem CreateTaskItem(string filePath) =>
        new()
        {
            Files =
            [
