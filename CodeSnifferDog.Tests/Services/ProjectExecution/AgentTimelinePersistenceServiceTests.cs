...
[TestClass]
public sealed class AgentTimelinePersistenceServiceTests
{
    ...
    {
        AgentTimelinePersistenceService service = new();
        ...
        AgentTimelineEntryMutationResult result = await service.AppendTimelineEntryAsync(
            dbContext,
            agentId,
    ...
    public async Task ToolStartedAndCompleted_MergeIntoSingleEntry()
    {
        ...
        AgentTimelineEntryMutationResult started = await service.AppendToolCallStartedEntryAsync(
            dbContext,
            agentId,
            CreateToolStarted("call-1", occurredAtUtc),
            TestContext.CancellationToken);
        await dbContext.SaveChangesAsync(TestContext.CancellationToken);
        AgentTimelineEntryMutationResult completed = await service.CompleteToolCallEntryAsync(
            dbContext,
            agentId,
    ...
    public async Task ToolReplay_UpdatesSameEntryWithoutNewSequence()
    {
        ...
        await service.CompleteToolCallEntryAsync(
            ...
            TestContext.CancellationToken);
        await dbContext.SaveChangesAsync(TestContext.CancellationToken);
        AgentTimelineEntryMutationResult replayed = await service.CompleteToolCallEntryAsync(
            dbContext,
            agentId,
    ...
    public async Task RemoveTranscriptEntries_RemovesOnlyNonInputEntriesAfterClearTime()
    {
        ...
        AgentTimelineRemovalMutationResult? result = await service.RemoveTranscriptEntriesAsync(
            dbContext,
            agentId,
