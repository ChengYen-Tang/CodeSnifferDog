...
[TestClass]
public sealed class ProjectQueueServiceTests
{
    ...
    public async Task QueueAsync_CreatesQueuedProjectAndReturnsUploadResult()
    {
        ...
        ProjectQueueService service = CreateService(dbContextFactory);
        Guid projectId = Guid.NewGuid();
        DateTimeOffset nowUtc = new(2026, 5, 15, 8, 0, 0, TimeSpan.Zero);

        ProjectUploadResult result = await service.QueueAsync(
            new ProjectQueueRequest(projectId, @"C:\upload\repo.zip", 123, @"uploads\repo.zip", nowUtc),
    ...
    public async Task QueueAsync_WhenQueueIsFull_ThrowsOriginalException()
    {
        ...
        ProjectQueueService service = CreateService(dbContextFactory, maxQueuedProjects: 1);

        InvalidOperationException exception = await Assert.ThrowsExactlyAsync<InvalidOperationException>(
            () => service.QueueAsync(
                new ProjectQueueRequest(Guid.NewGuid(), "repo.zip", 123, "uploads/repo.zip", DateTimeOffset.UtcNow),
                TestContext.CancellationToken));

        Assert.AreEqual("The project queue is full.", exception.Message);
    }

    private static ProjectQueueService CreateService(
