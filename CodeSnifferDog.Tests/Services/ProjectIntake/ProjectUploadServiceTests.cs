...
public sealed class ProjectUploadServiceTests
{
    ...
    {
        ProjectUploadService service = CreateService();
        Guid projectId = Guid.NewGuid();
        FormFile zipFile = CreateFormFile("repo.zip", "content");

        ProjectUploadArtifact artifact = await service.StoreAsync(projectId, zipFile, TestContext.CancellationToken);
    ...
    private static ProjectUploadService CreateService() =>
        new(new ProjectTemporaryStoragePaths(), NullLogger<ProjectUploadService>.Instance);
