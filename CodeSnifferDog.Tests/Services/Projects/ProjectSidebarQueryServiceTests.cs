...
[TestClass]
public sealed class ProjectSidebarQueryServiceTests
{
    ...
    public async Task GetSnapshotAsync_ReturnsFixedGroupsSortedProjectsAndSelectedFallback()
    {
        ...
        ProjectSidebarQueryService service = new(dbContextFactory, new ProjectStatusMapper());

        ProjectSidebarSnapshotReadModel snapshot = await service.GetSnapshotAsync(
    ...
    {
        ProjectSidebarQueryService service = new(CreateDbContextFactory(), new ProjectStatusMapper());

        ProjectSidebarSnapshotReadModel snapshot = await service.GetSnapshotAsync(null, TestContext.CancellationToken);
