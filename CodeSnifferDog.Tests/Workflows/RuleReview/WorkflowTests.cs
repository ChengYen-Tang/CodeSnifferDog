...
public sealed class WorkflowTests
{
    ...
    public async Task RunAsync_IgnoresLateWrites_FromTimedOutAttempt()
    {
        ...
        InMemoryIssueStore issueStore = new();
                ...
                Task backgroundWriteTask = Task.Run(async () =>
                {
                    try
                    {
                        ...
                        await issueStore.AddAsync(
                            RuleScopeKeyFactory.CreateRuleFlowKey(TestRepositoryPaths.RootPath, "task-item-1", PerformanceRuleFileName),
                            new Issue
                            {
    ...
    public async Task RunAsync_PreservesCompactionArtifacts_AndCompletesWorkflow()
    {
        ...
        AgentCompactionOptions compactionOptions = CreateCompactionOptions(
            RuleReviewAgentPromptAssets.RuleReviewSummaryPrompt,
            summarizer);
        ...
        Workflow workflow = new(
            (repositoryRootPath, ruleFileName, ruleMarkdown, taskItem, _) => new AgentFactory(compactionOptions).Create(
                reviewChatClient,
                repositoryRootPath,
                ruleFileName,
                ruleMarkdown,
                ...
                issueStore,
                verdictBuffer),
            (repositoryRootPath, ruleFileName, ruleMarkdown, taskItem, _) => new VerifierFactory(compactionOptions).Create(
                verifierChatClient,
                repositoryRootPath,
                ruleFileName,
                ruleMarkdown,
    ...
    private static AgentCreationResult CreateReviewAgent(
        ...
        new AgentFactory(CreateCompactionOptions(RuleReviewAgentPromptAssets.RuleReviewSummaryPrompt))
    ...
    private static AgentCreationResult CreateVerifierAgent(
        ...
        IIssueStore issueStore,
        ReviewVerdictBuffer verdictBuffer) =>
        new VerifierFactory(CreateCompactionOptions(RuleReviewAgentPromptAssets.RuleReviewSummaryPrompt))
            .Create(chatClient, repositoryRootPath, ruleFileName, ruleMarkdown, taskItem, issueStore, verdictBuffer);
